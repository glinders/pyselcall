...
class BaseTest(unittest.TestCase):
    ...
    def assertLow(self, value, minimum, tolerance=0.1):
        assert minimum <= value <= (minimum - (
                    minimum * tolerance)), "Low value {} is not within a minimum tolerance of {:%}".format(
            value, tolerance)

    def assertHigh(self, value, maximum, tolerance=0.1):
        assert (maximum - (
                    maximum * tolerance)) <= value <= maximum, "High value {} is not within a maximum tolerance of {:%}".format(
            value, tolerance)
