...
def run():
    parser = ArgumentParser(
        epilog='If the -f option is not given, then this script will attempt to play the sounds through pyaudio.')
    ...
    group.add_argument('-d', dest='duration', type=int, default=20,
                       help='duration in milliseconds')
    group.add_argument('-e', dest='duration', action='store_const',
                       const=pyselcall.Durations.ERICSSON,
                       help='Use Ericsson 700ms, 100ms, 100ms... durations')

    parser.add_argument('-a', dest='attennuation', type=int, default=2,
                        help='output volume attennuation, 1..n where 1 is full volume, defaults to 2')
    parser.add_argument('-f', dest='filename',
                        help='output audio to a wav file with the given filename')
    ...
    melody = pyselcall.Melody(pyselcall.Code(args.numbers, args.duration, standard),
                              waveform=waveform)
